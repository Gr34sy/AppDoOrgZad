...
Dynamiczny rozwój technologii internetowych oraz rosnąca liczba obowiązków zawodowych i prywatnych powodują, że efektywne zarządzanie czasem i zadaniami staje się istotnym elementem codziennego funkcjonowania użytkowników. W odpowiedzi na te potrzeby powstają liczne aplikacje, wspierające planowanie pracy, organizację projektów oraz monitorowanie postępów realizacji celów. Jednocześnie rozwój nowoczesnych frameworków frontendowych i backendowych pozwala na tworzenie wydajnych, skalowalnych i interaktywnych systemów dostępnych z poziomu przeglądarki internetowej.

Inspiracją dla tematu są popularne aplikacje do zarządzania projektami, kładące duży nacisk na organizację pracy zespołowej. Narzędzia takie jak Trello, Asana czy ClickUp dostarczają szeroką gamę funkcji, umożliwiających organizację zadań, jednak wiele z nich jest niepotrzebne użytkownikowi indywidualnemu. Nadmiar zbędnych funkcji może utrudniać takiemu użytkownikowi korzystanie z aplikacji. Dlatego też
w projekcie zdecydowano się na przeniesienie, z wyżej wymienionych aplikacji, najważniejszych funkcjonalności oraz narzędzi i zaadaptowanie ich dla potrzeb użytkowników indywidualnych. W prezentowanym projekcie dominują uproszczone wersje rozwiązań stosowanych w aplikacjach do organizacji projektów zespołowych.

System ma umożliwiać użytkownikom organizowanie pracy w sposób przejrzysty i intuicyjny, z wykorzystaniem funkcji takich jak tworzenie, edycja i usuwanie elementów, przypinanie wybranych treści do strony głównej oraz zarządzanie strukturą projektów. Istotnym założeniem projektu jest również zapewnienie bezpieczeństwa danych poprzez wdrożenie mechanizmów logowania, rejestracji oraz autoryzacji użytkowników.
...
Zakres realizowanych prac obejmuje analizę dostępnych na rynku rozwiązań, opis oraz analizę odpowiednich narzędzi programistycznych, przygotowanie graficznych projektów widoków interfejsu użytkownika oraz przedstawienie implementacji aplikacji. Zakres pracy nie obejmuje publikacji aplikacji w domenie publicznej ani jej komercyjnego wdrożenia.
Rezultatem pracy jest funkcjonalna aplikacja internetowa, stanowiąca przykład zastosowania nowoczesnych technologii frontendowych i backendowych w praktycznej realizacji systemu wspierającego zarządzanie zadaniami.
Cele szczegółowe niniejszej pracy obejmują zaprojektowanie graficznego wyglądu interfejsu użytkownika w sposób zapewniający intuicyjną obsługę, czytelność oraz estetykę. Kolejnym celem jest opracowanie logiki działania systemu zarówno po stronie klienckiej, odpowiedzialnej za interakcję z użytkownikiem, jak i po stronie serwerowej, realizującej logikę biznesową\footnote{Logika biznesowa - to zbiór reguł i mechanizmów aplikacji, określających sposób przetwarzania danych oraz realizacji operacji zgodnie z wymaganiami systemu..} oraz komunikację z bazą danych. Praca zakłada również zaprojektowanie struktury bazy danych dostosowanej do przechowywania i przetwarzania informacji o użytkownikach, zadaniach oraz projektach. Istotnym celem jest także wdrożenie mechanizmów logowania, rejestracji oraz autoryzacji użytkowników, zapewniających bezpieczny dostęp do zasobów aplikacji.

\section*{Streszczenie rozdziałów}

Rozdział pierwszy przedstawia analizę wymagań projektowanej aplikacji. Omówiono w nim potrzeby użytkownika indywidualnego, porównano podobne narzędzia do organizacji zadań oraz wskazano wymagania funkcjonalne i niefunkcjonalne systemu. W rozdziale opisano również historie użytkownika oraz podstawy metody Kanban, która została wykorzystana jako jeden ze sposobów organizacji zadań w projektach.

Rozdział drugi dotyczy technologii frontendowych zastosowanych podczas realizacji aplikacji. Przedstawiono w nim rolę języków HTML, CSS i JavaScript, a także opisano wykorzystanie biblioteki React, mechanizmu React Hooks, frameworka TailwindCSS oraz języka TypeScript. Rozdział ten wyjaśnia, w jaki sposób wskazane technologie wspierają budowę interaktywnego i responsywnego interfejsu użytkownika.

Rozdział trzeci opisuje technologie backendowe oraz mechanizmy odpowiedzialne za działanie aplikacji po stronie serwera. Szczególną uwagę poświęcono frameworkowi NextJS, routingowi, generowaniu zawartości strony, obsłudze endpointów API oraz procesom autoryzacji i autentykacji użytkowników.

Rozdział czwarty przedstawia zagadnienia związane z przechowywaniem danych. Omówiono w nim podstawowe cechy baz danych oraz uzasadniono wybór MongoDB jako rozwiązania dopasowanego do dokumentowego charakteru danych przetwarzanych przez aplikację.
...
Rozdział szósty prezentuje projekt interfejsu użytkownika oraz scenariusze przypadków użycia. Opisano w nim sposób korzystania z głównych funkcji systemu, takich jak obsługa konta użytkownika, praca z notatkami, checklistami, zadaniami i projektami oraz wykorzystanie widoku Kanban.

Rozdział siódmy opisuje wybrane elementy implementacji kodu frontendowego. Skupiono się w nim na realizacji widoku Kanban oraz sekcji przypiętych elementów dashboardu, ponieważ są to części aplikacji istotne dla codziennej pracy użytkownika i organizowania najważniejszych danych.

\chapter{Analiza wymagań}

W dobie rosnącej liczby obowiązków zawodowych, edukacyjnych i prywatnych coraz więcej osób poszukuje skutecznych narzędzi wspierających zarządzanie czasem, planowanie zadań i realizację celów. Wiele zaawansowanych aplikacji i systemów do zarządzania projektami takich jak Trello, Asana czy ClickUp zostało zaprojektowanych
z myślą o pracy zespołowej oraz dużych organizacjach. W konsekwencji, ich interfejsy bywają skomplikowane, a nadmiar funkcji może przytłaczać i zniechęcać użytkowników indywidualnych, którzy potrzebują prostoty, przejrzystości a przede wszystkim szybkiego dostępu do kluczowych funkcji.

Na rynku brakuje aplikacji, które w sposób zbalansowany łączą prostotę obsługi
z możliwością wyboru widoków list. Użytkownicy często zmuszeni są korzystać z kilku narzędzi jednocześnie -- jednej aplikacji do zarządzania codziennymi zadaniami, innej do planowania bardziej skomplikowanych projektów, a jeszcze innej do tworzenia krótkich notatek. Istnieje więc, potrzeba stworzenia prostej, nowoczesnej aplikacji internetowej, która odpowiadałaby użytkownikom indywidualnym, oferując przejrzysty sposób zarządzania projektami oraz prostymi obowiązkami życia codziennego.
...
\item
  umożliwiać szybką organizację zadań różnego typu - od codziennych spraw, jak lista zakupów, po wielozadaniowe projekty,
\item
  oferować prosty i motywujący do działania design.
\end{itemize}
...
Asana, ClickUp oraz Trello to przykładowe, popularne narzędzia do zarządzania projektami. Z perspektywy pojedynczego użytkownika ich przydatność zależy od złożoności potrzeb oraz preferowanego stylu pracy. Ogólnie \textbf{Trello} uchodzi za najprostsze, najbardziej intuicyjne narzędzie. Prostota interfejsu tej aplikacji umożliwia szybkie
i przyjemne poruszanie się po interfejsie. \textbf{Asana} natomiast, jest dużo bardziej rozbudowana. Korzystają z niej głównie duże zespoły oraz korporacje. Używa się jej do nadzoru bardziej złożonych projektów. Z kolei \textbf{ClickUp,} oferuje za darmo podobny zakres funkcji co płatny plan Asany, będąc przez to bardziej atrakcyjnym dla wymagających, indywidualnych użytkowników niż \textbf{Trello}. Liczne zaawansowane funkcje dostarczane przez ClickUp czynią jednak tą aplikację dość złożoną w obsłudze.

\textbf{Trello} wyróżnia się \textbf{prostotą i intuicyjnością} -- jest łatwe do opanowania
dla początkujących, dzięki czemu idealnie nadaje się do podstawowego porządkowania. Jego jedyna forma prezentacji - kanban sprzyja szybkiemu organizowaniu prostych projektów. Jednocześnie Trello pozostawia dużo przestrzeni użytkownikom bardziej wymagającym, poprzez możliwość integracji przeróżnych narzędzi zewnętrznych, tak zwanych Power-Upów.
...
Najważniejszym aspektem aplikacji jest czytelność i prostota obsługi interfejsu.

Użytkownik indywidualny nie potrzebuje licznych widoków listy zadań, zaawansowanej automatyzacji dodawania tasków, czy tworzenia formularzy zespołowych, usprawniających dodawanie zawartości do listy. Użytkownik taki potrzebuje natomiast szybko odnaleźć poszukiwane funkcje aplikacji, bądź sprawnie przejść z poziomu strony głównej do projektu, który aktualnie realizuje i sprawdzić jakie rzeczy pozostały mu jeszcze do wykonania. Dlatego w projekcie aplikacji będącej tematem tej pracy, interfejs
i funkcjonalności aplikacji zostały maksymalnie uproszczone.
...
3. Jako głowa rodziny, potrzebuję utworzyć check listę zakupów, aby w sklepie niczego nie zapomnieć.
...
Na podstawie powyższych historii użytkownika opracowano szczegółowe scenariusze przypadków użycia, opisujące sposób interakcji użytkownika z systemem
w konkretnych sytuacjach. Każda historia została przełożona na sekwencję kroków obejmujących działania użytkownika oraz oczekiwane reakcje aplikacji, co umożliwiło precyzyjne określenie przebiegu realizacji poszczególnych funkcjonalności.
...
\begin{table}[H]
  ...
  \begin{tabular}{|>{\raggedright\arraybackslash}p{2.55cm}|>{\raggedright\arraybackslash}p{2.55cm}|>{\raggedright\arraybackslash}p{2.55cm}|>{\raggedright\arraybackslash}p{2.55cm}|>{\raggedright\arraybackslash}p{2.55cm}|}
    ...
    \hline
    Pomysły na nowe funkcje & Przygotowanie formularza zadania & Implementacja widoku projektu & Sprawdzenie walidacji formularza & Konfiguracja logowania \\
    \hline
...
Na podstawie powyższego podrozdziału powstały wymagania funkcjonalne
i niefunkcjonalne, które spełnia aplikacja będąca tematem niniejszej pracy. Wymagania te stanowiły podstawę do opracowania architektury aplikacji, projektowania interfejsu użytkownika oraz implementacji poszczególnych modułów.
Podstawową funkcjonalnością systemu jest umożliwienie użytkownikowi kompleksowego zarządzania elementami organizacyjnymi, takimi jak checklisty, notatki, taski oraz projekty. Aplikacja wspiera pełen zestaw operacji CRUD\footnote{CRUD (z jęz. ang. Create, Read, Update, Delete) - zestaw podstawowych operacji wykonywanych na danych w systemach informatycznych, obejmujący ich tworzenie, odczyt, modyfikację oraz usuwanie.}, co pozwala na swobodne dodawanie elementów, ich edycję oraz usuwanie w zależności od potrzeb użytkownika.
Istotnym elementem systemu jest możliwość prezentacji projektów w formie tablicy Kanban\footnote{Kanban - metoda wizualnego zarządzania pracą wywodząca się z metodyk zwinnych, polegająca na przedstawieniu zadań w formie tablicy z kolumnami reprezentującymi etapy realizacji.}. Rozwiązanie to pozwala na wizualne przedstawienie stanu realizacji zadań poprzez podział na kolumny, odpowiadające poszczególnym etapom pracy
...
\item
  Interakcje - wypełnianie formularzy, klikanie w przycisk, menu nawigacyjne
\item
  Responsywność - automatyczne dostosowywanie się aplikacji do różnych wielkości ekranu
\end{enumerate}

Fundamentalne narzędzia używane do tworzenia frontendu to HTML\footnote{HTML - z jęz. ang. Hypertext Markup Language, czyli ``hipertekstowy język znaczników''.}, CSS\footnote{CSS - z jęz. ang. Cascading Style Sheets, oznaczające ``kaskadujące arkusze stylów''.} oraz JavaScript\textbf{.} HTML to język znaczników, odpowiadający za strukturę strony internetowej. Określa on semantyczny układ treści, takich jak nagłówki, akapity, listy, obrazy czy formularze. HTML stanowi szkielet warstwy frontendowej aplikacji webowej, będąc interpretowanym przez przeglądarkę w celu prezentacji treści użytkownikowi. Odpowiednie ustawienie Poprawne i semantyczne rozmieszczenie znaczników HTML ma kluczowe znaczenie dla dostępności cyfrowej, ponieważ umożliwia prawidłową interpretację treści przez czytniki ekranu wykorzystywane przez osoby
...
Bowiem wczesne zastosowania HTML, CSS i JavaScript ograniczały się głównie do prostych, statycznych stron. Jednak wraz ze wzrostem ilości aplikacji interaktywnych, pojawiła się konieczność lepszej organizacji kodu, ponownego wykorzystania komponentów oraz efektywniejszego zarządzania stanem aplikacji. Dynamiczny rozwój stron internetowych względem ich funkcjonalności i użyteczności zmusił programistów do usprawnienia wcześniej wymienionych języków, w celu sprostania rosnącym oczekiwaniom użytkowników. Biblioteki powstały jako odpowiedź na te wyzwania, umożliwiając realizację dużo bardziej ambitnych i zaawansowanych aplikacji internetowych.
...
Najważniejszym przykładem jest tutaj React, czyli biblioteka języka JavaScript.

Jak można wyczytać na oficjalnej stronie dokumentacji biblioteki\footnote{\emph{React the library for web and native interfaces}, \url{https://react.dev} (dostęp: 28.01.2026).} dostarcza ona narzędzia, pozwalająca między innymi na tworzenie komponentów nadających się do ponownego użycia w różnych sekcjach aplikacji. W ten sposób można stworzyć na przykład przycisk, bądź nagłówek, bez potrzeby kilkukrotnego przepisywania kodu, gdy chcemy by nasz dokument HTML zawierał kilka takich samych elementów. Takie podejście nie tylko oszczędza czas, ale również znacznie poprawia przejrzystość kodu. Właśnie z tego powodu w niniejszej pracy inżynierskiej zdecydowano się na skorzystanie z tej biblioteki.
...
i umożliwiają podział aplikacji na mniejsze, niezależne moduły. Takie podejście redukuje powielanie kodu oraz ułatwia jego testowanie i utrzymanie. Modułowa struktura aplikacji reactowej pozwala na lepszą organizację, zwiększając zarówno czytelność kodu, jak
i skalowalność projektu. Jednak wyżej wspomniane funkcje Reacta nie byłyby możliwe dzięki nakładce do języka JavaScript zwanej JavaScript Extension - w skrócie JSX.

Jako, że HTML jest językiem deklaratywnym służącym do opisu struktury dokumentu, natomiast JavaScript odpowiada za logikę i manipulację tą strukturą.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=5.66892in,height=0.80294in]{imported-media/media/image1.png}
  \caption{fragment kodu przedstawiający deklaratywne definiowanie komponentu Button dzięki JSX}
\end{figure}

Wyżej przedstawiono, jak prosto w sposób deklaratywny dodaje się elementy HTMLa, dzięki bibliotece React. Na powyżej zamieszczonych przykładach można zaobserwować, jak ogromna jest różnica pomiędzy wspomnianymi metodami obsługiwania znaczników HTML w JavaScripcie.
W React wyróżnia się dwa podstawowe typy komponentów - klasowe oraz funkcyjne, różniące się zarówno składnią, jak i sposobem zarządzania logiką aplikacji. Komponenty klasowe opierają się na klasach wprowadzonych do JavaScript wraz
...
\begin{figure}[H]
  \centering
  \includegraphics[width=4.61253in,height=2.52232in]{imported-media/media/image4.png}
...
W komponentach funkcyjnych stan jest obsługiwany za pomocą funkcji stanu, składającej się z aktualnej wartości stanu oraz funkcji służącej do jej zmiany. Funkcja ta nie modyfikuje stanu bezpośrednio, lecz informuje Reacta o konieczności jego aktualizacji, co pozwala zachować spójność danych i przewidywalność działania aplikacji.

\begin{figure}[H]
  \centering
  \includegraphics[width=5.83952in,height=3.23864in]{imported-media/media/image9.png}
...
Takie podejście zapewnia enkapsulację danych wewnątrz komponentu oraz precyzyjne sterowanie jego zachowaniem. Dzięki hookom zarządzanie stanem staje się czytelne i ściśle powiązane z zakresem funkcji komponentu, co ułatwia rozwój
i utrzymanie aplikacji frontendowych.

\section{TailwindCSS}

Z oficjalnej strony\footnote{\emph{Strona Tailwindcss}, \url{https://tailwindcss.com} (dostęp: 28.01.2026).} możemy się dowiedzieć, że jest to framework, służący do stylowania interfejsów użytkownika za pomocą gotowych klas CSS, używanych bezpośrednio
...
\begin{figure}[H]
  \centering
  \includegraphics[width=6.10236in,height=2.61111in]{imported-media/media/image6.png}
...
Dla przykładu - klasa ``flex''nadana znacznikowi ``div'' w piątej linijce kodu, jest tożsama z CSSowym ``display: flex;''. W niniejszej pracy inżynierskiej Tailwinda użyto przy projektowaniu aplikacji, z następujących powodów:
...
Kolejną technologią zastosowaną przy projekcie aplikacji jest TypeScript, czyli statycznie typowany nadzbiór języka JavaScript. Rozszerza on JSa o system typów, interfejsy oraz mechanizmy kontroli poprawności kodu na etapie kompilacji. Kod TypeScript nie jest bezpośrednio interpretowany przez przeglądarkę, lecz przed uruchomieniem musi zostać transpilowany do standardowego JavaScriptu, zachowując pełną kompatybilność ze środowiskiem wykonawczym.
...
Domyślnie NextJS używa serwerowych komponentów Reacta. Wykonują się one najpierw po stronie serwera, a następnie ich wynik wysyłany jest do klienta.
...
Zastosowanie API Routes obejmuje m.in. obsługę formularzy, operacje CRUD\footnote{CRUD - z jęz. ang. skrót odnoszący się do wyrazów: Create, Read, Update, Delete. Oznacza możliwość tworzenia, odczytu, aktualizacji oraz usuwania danych.}, integracje z usługami zewnętrznymi, czy realizację logiki po stronie serwera, która nie powinna być wykonywana w przeglądarce. Istotną zaletą jest fakt, że kod endpointów nie trafia do klienta, dzięki czemu możliwe jest bezpieczniejsze przetwarzanie danych oraz użycie zmiennych środowiskowych po stronie serwera.
...
\section{Uzasadnienie wyboru NextJS}

W niniejszej pracy dyplomowej, zdecydowano się na skorzystanie z NextJSa z uwagi na szereg narzędzi, udostępnianych przez niego, które wymieniono we wcześniejszych podrozdziałach. Next.js stanowi doskonałe środowisko do budowy aplikacji internetowych opartych o React, zapewniając jednocześnie wygodne narzędzia typowe dla warstwy backendowej. Poniżej przedstawiono kluczowe argumenty przemawiające za jego wykorzystaniem.
Jedną z istotnych zalet Next.js jest wsparcie dla różnych strategii renderowania, co pozwala dopasować sposób generowania treści do charakteru poszczególnych ekranów aplikacji. W kontekście aplikacji do organizacji zadań większość treści znajduje się za panelem logowania, jednak nadal istnieją elementy, które mogą korzystać z poprawionej indeksowalności oraz szybszego wyświetlania strony, jak chociażby strona logowania.
Renderowanie po stronie serwera umożliwia dostarczenie kompletnego HTML już na etapie odpowiedzi serwera, co ułatwia pracę algorytmom indeksującym, a także skraca czas potrzebny do pierwszego wyświetlenia treści. Dodatkowo Next.js ułatwia zarządzanie metadanymi strony(np. tytuł, opis), co przekłada się na spójne i przewidywalne zachowanie aplikacji w przeglądarkach oraz w serwisach społecznościowych. Z punktu widzenia pracy inżynierskiej ważne jest to, że framework oferuje mechanizmy SEO wbudowane w standardowy sposób tworzenia aplikacji, bez konieczności ręcznego konfigurowania.
Aplikacja do zarządzania zadaniami posiada ogromny potencjał do konsekwentnego rozwijania jej wraz z dodawaniem nowych funkcjonalności. Next.js sprzyja umożliwia skalowalność na kilku poziomach, lecz przede wszystkim zapewnia czytelną strukturę projektu, dzięki routingowi opartemu o system plików wraz z możliwościami wydzielania modułów. Next.js pozwala rozwijać aplikację etapami, utrzymując spójny model wytwarzania, natomiast wraz ze wzrostem złożoności projektu nie zachodzi potrzeba migracji na inne narzędzia.
Kolejnym argumentem jest możliwość budowy zarówno warstwy prezentacji, jak
i elementów backendowych w ramach jednego frameworka. Next.js dostarcza mechanizmy takie jak endpointy API, middleware\footnote{Middleware -- warstwa pośrednia oprogramowania działająca pomiędzy żądaniem użytkownika a właściwą logiką aplikacji, umożliwiająca przechwytywanie i modyfikację zapytań oraz odpowiedzi. Stosowana jest m.in. do realizacji takich zadań jak autentykacja, autoryzacja, logowanie zdarzeń, obsługa błędów czy przekierowania, bez ingerencji w główną logikę biznesową systemu.} oraz integrację z logiką serwerową, co jest szczególnie korzystne w wymagających aplikacjach. Takie podejście mocno skraca czas implementacji, ponieważ eliminuje konieczność utrzymywania osobnego projektu backendowego. Zmniejsza to również ryzyko niespójności kontraktów API oraz upraszcza jej wdrażanie, gdyż aplikacja może być publikowana jako całość.
Istotną zaletą Nexta jest również integracja z biblioteką \href{http://auth.js}{Auth.js}. Pozwala to implementować zabezpieczenia zarówno po stronie klienta - jak warunkowe renderowanie widoków, oraz po stronie serwera - ochrona endpointów API, czy walidacja sesji
w middleware. Dzięki temu kontrola dostępu nie jest oparta wyłącznie o logikę frontendową, lecz może być egzekwowana na poziomie serwera, co zwiększa bezpieczeństwo rozwiązania.
Zastosowanie Auth.js eliminuje również konieczność budowania całego mechanizmu logowania od podstaw. Stworzenie logiki, zajmującej się obsługiwaniem sesji, tokenów, odświeżania, błędów, integracji z dostawcami autentykacji i autoryzacji, zajmuje dużo czasu. Jednocześnie rozwiązania zastosowane w tej bibliotece pozostają elastyczne i możliwe do rozbudowy.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=5.7792in,height=2.98542in]{imported-media/media/image10.png}
...
z bazą odbywa się poprzez zapytania realizowane w formie obiektów lub metod API, zamiast klasycznych zapytań w języku SQL. Do kluczowych możliwości MongoDB należą:

elastyczny schemat danych (schema-less),

przechowywanie danych w strukturze dokumentowej w formacie JSON lub BSON\footnote{BSON - skrót od binary JSON, binarna reprezentacja formatu JSON.},

wysoka skalowalność pozioma\footnote{Skalowanie poziome - metoda zwiększania wydajności i pojemności systemu poprzez dodawanie kolejnych serwerów lub komputerów (zwanych węzłami), zamiast rozbudowy pojedynczej maszyny.},

wydajne operacje odczytu i zapisu dla dużych zbiorów danych,

zaawansowane mechanizmy filtrowania\footnote{Filtrowanie - operacja na danych, polegająca na selekcji rekordów spełniających określone warunki} i agregacji\footnote{Agregacja - operacja na danych, polegająca na łączeniu i podsumowywaniu danych w celu uzyskania zbiorczych wyników, takich jak sumy, średnie lub grupowanie wyników.} danych.

MongoDB znajduje zastosowanie w projektach, w których dane mają zmienną strukturę lub bardzo dużą skalę. Jako przykłady można podać systemy zarządzania treścią, systemy analityczne, operujące na dużych ilościach danych lub aplikacje mobilne oraz internetowe, charakteryzujące się dynamicznie zmieniającymi się modelami danych. MySQL sprawdza się lepiej w systemach wymagających ścisłych relacji i spójności danych, natomiast MongoDB oferuje większą elastyczność i łatwiejsze skalowanie.

\section{Uzasadnienie wyboru MongoDB}

W projekcie aplikacji, będącej tematem niniejszej pracy, kluczowe znaczenie ma łatwość rozbudowy funkcjonalności oraz możliwość obsługi dynamicznie zmieniających się struktur informacji. Z tego względu zdecydowano się na zastosowanie bazy dokumentowej MongoDB zamiast klasycznej relacyjnej bazy danych.
...
Ponadto MongoDB znakomicie integruje się z technologiami JavaScript, co sprzyja spójności technologicznej projektu i upraszcza proces programowania. NextJS, będący częścią stosu technologicznego projektu aplikacji, zapewnia dobre wsparcie dla narzędzi oferowanych przez MongoDB.

\chapter{Architektura aplikacji}

Architektura klient-serwer jest jednym z podstawowych modeli projektowania aplikacji internetowych, w którym system podzielony jest na dwie warstwy - klienta oraz serwera. Klient odpowiada za interakcję z użytkownikiem i prezentację danych, natomiast serwer realizuje logikę biznesową, przetwarzanie danych oraz komunikację z bazą danych.
W tym modelu komunikacja odbywa się w schemacie ``żądanie - odpowiedź''. Klient wysyła do serwera zapytanie, np. o pobranie danych lub zapis informacji, a serwer przetwarza je i zwraca odpowiedź w postaci danych lub komunikatu o wyniku operacji. Takie rozdzielenie odpowiedzialności pozwala oddzielić warstwę prezentacji od warstwy przetwarzania danych, co zwiększa przejrzystość projektu oraz ułatwia jego rozwój.
...
Architektura ta stanowi fundament współczesnych aplikacji internetowych, umożliwiając tworzenie systemów modularnych, wydajnych i łatwych w utrzymaniu, co ma szczególne znaczenie w projektach o rosnącej złożoności i liczbie użytkowników.
Rozpatrzmy funkcjonowanie architektury klient-serwer w przypadku projektu aplikacji do organizowania zadań. Warstwa klienta, realizowana w przeglądarce internetowej, odpowiada za prezentację interfejsu użytkownika, obsługę interakcji oraz wysyłanie żądań dotyczących operacji na danych, takich jak dodawanie, edycja czy usuwanie zadań. Warstwa serwerowa przetwarza te żądania, realizuje logikę biznesową aplikacji, zarządza autentykacją użytkowników oraz komunikuje się z bazą danych,
w której przechowywane są informacje o projektach i zadaniach.
W systemie wymagającym logowania użytkownika, szczególne znaczenie musi mieć bezpieczeństwo. Dzięki rozdzieleniu warstw, klient nie posiada bezpośredniego dostępu do bazy danych, ani wrażliwych mechanizmów aplikacji. Wszelkie operacje wykonywane są po stronie serwera, gdzie możliwa jest weryfikacja tożsamości użytkownika, kontrola uprawnień oraz walidacja przesyłanych danych. Pozwala to zapobiegać nieautoryzowanemu dostępowi do cudzych zasobów, a także ogranicza ryzyko manipulacji danymi lub ataków polegających na modyfikacji kodu po stronie przeglądarki. Zmniejsza to również ryzyko wycieku danych poufnych i dostania się ich w niepowołane ręce.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=6.10236in,height=4.11111in]{imported-media/media/image3.png}
...
Warstwa prezentacji odpowiada za interfejs użytkownika (UI) oraz sposób prezentacji danych. Wyświetla widoki formularzy, zbiera dane wejściowe, które potem mogą zostać przesłane w postaci zapytań do bazy danych oraz zajmuje się podstawową walidacją\footnote{Walidacja - proces sprawdzania poprawności, kompletności oraz zgodności danych wejściowych z określonymi regułami i wymaganiami systemu.} formularzy. Odpowiada również za wysyłanie zapytań do backendu.
...
Źródła zewnętrze z kolei to zewnętrzne systemy i usługi, z którymi aplikacja się integruje. Mogą to być zewnętrzne bazy lub pliki z danymi (data sources), integracje
...
\begin{figure}[H]
  \centering
  \includegraphics[width=5.53778in,height=3.10511in]{imported-media/media/image13.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=5.47191in,height=4.70023in]{imported-media/media/image2.png}
...
W bazie zastosowano indeksy wspierające najczęstsze operacje aplikacji. Większość kolekcji posiada indeks po polu właściciela, co przyspiesza pobieranie danych aktualnego użytkownika. Dodatkowe indeksy obejmują między innymi pozycję sortowania, tagi, termin wykonania, priorytet, status zadania oraz pola wykorzystywane do przypięć. Dzięki temu aplikacja może sprawnie filtrować zadania i projekty, sortować elementy dashboardu oraz szybko sprawdzać, czy dany element został już przypięty.

Usuwanie większości elementów domenowych zostało zrealizowane jako archiwizacja. Zadania, projekty i checklisty otrzymują wartość w polu \texttt{archivedAt}, a zapytania listujące pobierają wyłącznie dokumenty aktywne. W przypadku projektu podczas archiwizacji ustawiany jest również status cyklu życia \texttt{archived}. Takie podejście pozwala zachować spójność powiązań i historię zmian bez fizycznego usuwania dokumentów. Model dokumentowy MongoDB jest w tym projekcie uzasadniony, ponieważ dane mają strukturę elastyczną, a część informacji, takich jak elementy checklisty i kolumny Kanban, jest naturalnie powiązana z dokumentem nadrzędnym i może być przechowywana bezpośrednio w jego strukturze.

\chapter{Projekt interfejsu użytkownika}

Jak już wcześniej ustalono, projekt interfejsu użytkownika, jest jednym z najważniejszych aspektów aplikacji. Przejrzystość, czytelność oraz prostota obsługi decydują w dużej mierze o użyteczności niniejszej aplikacji. W tym celu najpierw rozpisane zostały historie użytkownika, po których analizie powstał graficzny projekt interfejsu.

\section{Scenariusze przypadków użycia}

\textbf{1)} \textbf{Przypięcie elementu do strony głównej}

\textbf{Aktor:} Użytkownik indywidualny

\textbf{Cel:} Szybki dostęp do śledzonych tasków/notatek/list/projektów z poziomu strony głównej
...
\def\labelenumi{\arabic{enumi}.}
\item
  Użytkownik otwiera którąś z list elementów(taski/listy/projekty itd.)
\item
...
\item
  ...
  }5a. Użytkownik pomija nazwę zadania 9a. System blokuje zapis i wskazuje brakujące pola.
\item
...
\begin{figure}[H]
  \centering
  \includegraphics[width=4.9in]{imported-media/media/image7.png}
  \caption{Diagram UML, wizualizujący scenariusz przypięcia taska}
\end{figure}

3) Utworzenie listy zakupów z możliwością odhaczanie pozycji
...
\item
  Użytkownik ustawia nazwę listy
\item
  Użytkownik dodaje pozycje w liście (np. mleko, chleb, warzywa)
\item
  System zapisuje listę i wyświetla ją użytkownikowi.
\item
  W sklepie użytkownik otwiera listę
\item
  Użytkownik odhacza kolejne pozycje
\item
  System zapisuje, które przedmioty w liście zostały odhaczone
\end{enumerate}
...
\item
  ...
  8a. System aktualizuje listę.
\end{enumerate}

\textbf{Warunki końcowe:} Lista zakupów istnieje, a pozycje mogą być odhaczane i zapisywane.
...
\item
  \textbf{Brak wymaganych danych (np. nazwy)
  }4a. Użytkownik pomija nazwę projektu 12a. System blokuje zapis i wskazuje brakujące pola.
\item
  \textbf{Niepoprawna data realizacji
...
{Dean J., \emph{Web programming with HTML, CSS, and JavaScript}, Burlington 2017}

Hasan Imam M., Chowdhury M., \emph{International Journal of Scientific Interdisciplinary Research}, „\emph{To what extent does the use of project management - oriented digital collaboration tools affect delivery timelines in remote tech teams?}'' 2025, tom 1, nr 1, s. 163--184.

{Mushaf, Nur M., Ilmi N., Nurfadilah, \& Shiddiq A., \emph{Journal of Embedded Systems, Security and Intelligent Systems}, ``Development of a Web-Based Scheduling Information System with Agile Methodology Using Next.js and Supabase'', 2025, tom \emph{6}, nr 2, s. 279--298}
