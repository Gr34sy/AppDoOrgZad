...
\FirstParagraph Ścieżki API w Next.js to mechanizm, umożliwiający definiowanie endpointów\footnote{Endpoint - z jęz. ang. punkt końcowy, ...} API bezpośrednio w projekcie Next.js. Punkty końcowe można tworzyć poprzez umieszczenie plików w katalogu ``\texttt{pages/api}'', a każda taka funkcja obsługuje żądania HTTP i zwraca odpowiedź w modelu ``\texttt{request/response}''\footnote{Interfejs typu request/response -- model komunikacji pomiędzy klientem a serwerem, w którym klient wysyła żądanie (request) zawierające określone dane lub operację, a serwer przetwarza je i zwraca odpowiedź (response) z wynikiem działania, danymi lub informacją o błędzie.}.
