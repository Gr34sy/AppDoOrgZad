...
\item
  wdrożenie mechanizmów logowania, obsługi kont użytkowników oraz autoryzacji, zapewniających bezpieczny dostęp do zasobów aplikacji.
\end{itemize}
...
Rozdział siódmy opisuje wybrane elementy implementacji kodu warstwy interfejsu użytkownika. Skupiono się w nim na wspólnych komponentach interfejsu, realizacji widoku tablicy projektu, sekcji przypiętych elementów dashboardu oraz formularzy odpowiedzialnych za tworzenie i edycję tasków oraz projektów, ponieważ są to części aplikacji istotne dla codziennej pracy użytkownika.

Rozdział ósmy przedstawia wybrane elementy implementacji backendu. Opisano w nim handler API odpowiedzialny za pobieranie, tworzenie, aktualizowanie i archiwizowanie checklist, wspólne mechanizmy obsługi żądań API oraz konfigurację autoryzacji opartą na Auth.js, zewnętrznych dostawcach logowania i sesji użytkownika.
...
\FirstParagraph W dobie rosnącej liczby obowiązków zawodowych, edukacyjnych i prywatnych coraz więcej osób poszukuje skutecznych narzędzi wspierających zarządzanie czasem, planowanie zadań i realizację celów. Wiele aplikacji i systemów do zarządzania zadaniami, takich jak Trello, Asana czy ClickUp, zostało zaprojektowanych z myślą o pracy zespołowej oraz dużych organizacjach. W konsekwencji ich interfejsy bywają skomplikowane, a nadmiar funkcji może przytłaczać i zniechęcać użytkowników indywidualnych, którzy wymagają prostoty, przejrzystości, a przede wszystkim szybkiego dostępu do kluczowych funkcji. Użytkownik indywidualny nie potrzebuje licznych widoków listy zadań, zaawansowanej automatyzacji dodawania tasków ani formularzy zespołowych usprawniających dodawanie zawartości do listy. Potrzebuje natomiast szybko odnaleźć funkcje aplikacji, sprawnie przejść ze strony głównej do realizowanego projektu i sprawdzić, które elementy pozostały jeszcze do wykonania.

Na rynku brakuje aplikacji, które w sposób zbalansowany łączą prostotę obsługi z możliwością wyboru sposobu organizacji zadań. Użytkownicy często zmuszeni są korzystać z kilku narzędzi jednocześnie -- jednej aplikacji do zarządzania codziennymi zadaniami, innej do planowania bardziej skomplikowanych projektów, a jeszcze innej do tworzenia krótkich notatek. Istnieje więc potrzeba stworzenia prostej, nowoczesnej aplikacji internetowej, która odpowiadałaby użytkownikom indywidualnym, oferując przejrzysty sposób zarządzania i organizacji rozmaitych zadań.
...
\textbf{ClickUp} spośród analizowanej trójki dostarcza bardzo szeroki zestaw funkcji. Zaawansowane widoki, zależności między zadaniami, śledzenie czasu pracy, raporty oraz liczne integracje sprawiają, że jest to aplikacja rozbudowana i elastyczna. Taka obfitość narzędzi wpływa jednak na złożoność interfejsu ClickUp. Korzystając z niego, użytkownik musi poświęcić więcej uwagi na odnalezienie potrzebnej funkcji, a opanowanie wszystkich możliwości aplikacji może wymagać dodatkowego czasu. Rozpatrując narzędzie w kontekście użytkownika indywidualnego, ClickUp może być atrakcyjniejszy od Asany ze względu na zakres dostępnych funkcji, natomiast wielu użytkownikom może bardziej odpowiadać Trello ze względu na prostotę.
...
Fundamentalne narzędzia używane do tworzenia frontendu to HTML\footnote{HTML - z jęz. ang. Hypertext Markup Language, czyli ``hipertekstowy język znaczników''. Dean J., \emph{Web Programming with HTML5, CSS, and JavaScript}, Jones \& Bartlett Learning, Burlington 2017, s. 7--15.}, CSS\footnote{Tamże, s. 73--75.} oraz JavaScript\footnote{Tamże, s. 311--313.}. HTML to język znaczników, odpowiadający za strukturę strony internetowej. Określa on semantyczny układ treści, takich jak nagłówki, akapity, listy, obrazy, czy formularze. HTML stanowi szkielet warstwy frontendowej aplikacji webowej, będąc interpretowanym przez przeglądarkę w celu prezentacji treści użytkownikowi. Poprawne i semantyczne rozmieszczenie znaczników HTML ma kluczowe znaczenie dla dostępności cyfrowej, ponieważ umożliwia prawidłową interpretację treści przez czytniki ekranu wykorzystywane przez osoby z niepełnosprawnościami wzroku.
...
Przykładem jest tutaj React\footnote{React, \emph{React: The Library for Web and Native Interfaces}, dokumentacja biblioteki React, [b.r.], dostęp: \url{https://react.dev} (28.01.2026).}, czyli biblioteka języka JavaScript.

Zgodnie z dokumentacją biblioteki React dostarcza ona narzędzia pozwalające między innymi tworzyć komponenty nadające się do ponownego użycia w różnych sekcjach aplikacji. W ten sposób można przygotować na przykład przycisk lub nagłówek bez potrzeby wielokrotnego przepisywania tego samego kodu. Takie podejście oszczędza czas oraz poprawia przejrzystość kodu. Z tego powodu w projekcie zdecydowano się na wykorzystanie tej biblioteki.
...
Poniżej zamieszczono przykład tego samego komponentu ``\texttt{Button}'', lecz w formie funkcji.
...
\FirstParagraph Mechanizm hooków\footnote{React, \emph{Built-in React Hooks}, dokumentacja hooków biblioteki React, [b.r.], dostęp: \url{https://react.dev/reference/react/hooks} (16.08.2026).} umożliwia przechowywanie danych pomiędzy kolejnymi wywołaniami funkcji komponentu, mimo że sama funkcja jest wykonywana od nowa przy każdym renderowaniu. Każdy hook jest wywoływany bezpośrednio w ciele funkcji komponentu, dzięki czemu React może jednoznacznie przypisać jego działanie do danego komponentu oraz zachować odpowiednie wartości pomiędzy kolejnymi wywołaniami tej funkcji. Kolejność wywołań hooków ma kluczowe znaczenie, ponieważ na jej podstawie React identyfikuje, które dane należą do danego hooka.
...
Dla przykładu klasa ``\texttt{flex}'' nadana znacznikowi ``\texttt{div}'' jest odpowiednikiem zapisu ``\texttt{display: flex;}'' w CSS. W projekcie aplikacji omawianej w niniejszej pracy Tailwind CSS wykorzystano z następujących powodów:

\begin{itemize}
\item
  ogranicza potrzebę definiowania własnych klas CSS,
\item
...
W aplikacji Tailwind CSS wykorzystano do przygotowania układów formularzy, kart elementów, bocznej nawigacji, widoku dashboardu oraz tablicy kanban. Klasy narzędziowe pozwoliły zachować spójne odstępy, kolory, responsywność i warianty trybu ciemnego bez tworzenia osobnych plików stylów dla każdego widoku.

\section{TypeScript}

\FirstParagraph Kolejną technologią zastosowaną w projekcie jest TypeScript\footnote{Microsoft, \emph{TypeScript Documentation}, dokumentacja języka TypeScript, [b.r.], dostęp: \url{https://www.typescriptlang.org/docs/} (25.07.2026).}, czyli nadzbiór języka JavaScript. Rozszerza on JavaScript o system typów, interfejsy oraz mechanizmy kontroli poprawności kodu na etapie kompilacji. Kod TypeScript nie jest bezpośrednio interpretowany przez przeglądarkę, lecz przed uruchomieniem musi zostać transpilowany do standardowego JavaScriptu, zachowując pełną kompatybilność ze środowiskiem wykonawczym.
...
\FirstParagraph Next.js\footnote{Vercel, \emph{Next.js Docs}, dokumentacja frameworka Next.js, [b.r.], dostęp: \url{https://nextjs.org/docs} (29.01.2026).} jest frameworkiem opartym na bibliotece React, przeznaczonym do tworzenia pełnostosowych aplikacji webowych. Interfejs użytkownika budowany jest przy użyciu komponentów React, natomiast Next.js dostarcza dodatkowe funkcje oraz mechanizmy optymalizacyjne. Framework automatycznie konfiguruje narzędzia niskiego poziomu, takie jak bundlery oraz kompilatory, dzięki czemu programista może skupić się na tworzeniu aplikacji i szybkim wdrażaniu jej do użytku.

Next.js rozszerza możliwości React o funkcje takie jak renderowanie zawartości po stronie serwera, routing\footnote{Routing - mechanizm zarządzania nawigacją w aplikacji, umożliwiający użytkownikowi poruszanie się pomiędzy różnymi częściami systemu.} oparty na strukturze plików, optymalizację obrazów oraz wygodne tworzenie endpointów API. Umożliwia również budowę aplikacji przyjaznych dla wyszukiwarek internetowych, ponieważ część treści może zostać przygotowana przed wysłaniem strony do przeglądarki. Dzięki temu framework łączy w jednym projekcie warstwę interfejsu użytkownika i wybrane mechanizmy backendowe.

W projekcie zdecydowano się na wykorzystanie Next.js, ponieważ framework ogranicza potrzebę utrzymywania osobnego projektu backendowego i ułatwia rozwijanie aplikacji jako jednej spójnej całości. Najważniejsze znaczenie miały routing oparty na strukturze plików, możliwość tworzenia endpointów API w tym samym repozytorium oraz wsparcie dla różnych strategii renderowania. Istotne było także proste połączenie z Auth.js\footnote{Auth.js, \emph{Auth.js}, dokumentacja biblioteki do obsługi uwierzytelniania, [b.r.], dostęp: \url{https://authjs.dev} (17.07.2026).}, które pozwala zabezpieczać widoki i endpointy bez tworzenia całego mechanizmu logowania od podstaw.
...
Dodatkowo framework wspiera nawigację po stronie klienta, realizowaną za pomocą komponentu ``\texttt{Link}'', dzięki czemu przejścia pomiędzy stronami odbywają się bez pełnego przeładowania dokumentu. Rozwiązanie to zwiększa wydajność oraz poprawia płynność działania interfejsu użytkownika.
...
\FirstParagraph Domyślnie Next.js używa serwerowych komponentów React. Wykonują się one najpierw po stronie serwera, a następnie ich wynik wysyłany jest do klienta.

Komponenty te zapewniają wsparcie dla obiektów typu Promise w JavaScripcie, umożliwiających wykonywanie operacji asynchronicznych bez konieczności używania dodatkowych bibliotek czy hooków React. Pobieranie danych odbywa się asynchronicznie, a ponieważ komponenty serwerowe działają po stronie serwera, możliwe jest bezpośrednie wykonywanie zapytań do bazy danych bez konieczności korzystania z dodatkowej warstwy API. Pozwala to ograniczyć ilość kodu, który trzeba pisać i utrzymywać.
...
Poniższy rysunek ogólnie pokazuje udział frontendu, backendu i bazy danych w procesie pobierania oraz prezentowania informacji użytkownikowi.
...
Dzięki swojej niezawodności oraz dobrej integracji z popularnymi technologiami backendowymi MySQL stanowi uniwersalne i efektywne rozwiązanie dla aplikacji wymagających uporządkowanego modelu danych oraz wysokiej spójności informacji. System ten dobrze sprawdza się w aplikacjach administracyjnych, systemach rezerwacji, portalach społecznościowych czy sklepach internetowych.

W niniejszej pracy MySQL został opisany jako punkt odniesienia dla technologii finalnie wykorzystanej w projekcie, czyli MongoDB. Porównanie obu podejść pozwala wskazać różnicę między relacyjnym modelem tabel a dokumentowym modelem kolekcji i dokumentów. W przypadku projektowanej aplikacji większe znaczenie miała elastyczność struktury danych, dlatego wybrano MongoDB, natomiast MySQL pozostaje przykładem rozwiązania właściwego dla systemów o bardziej stałych i silnie powiązanych relacjach.

\section{MongoDB}

\FirstParagraph MongoDB\footnote{MongoDB, Inc., \emph{Welcome to the MongoDB Docs}, dokumentacja systemu MongoDB, [b.r.], dostęp: \url{https://www.mongodb.com/docs/} (02.02.2026).} jest nierelacyjnym systemem zarządzania bazą danych, którego podstawową różnicą względem relacyjnych rozwiązań, jest sposób przechowywania informacji. MongoDB wykorzystuje model dokumentowy, w którym dane zapisywane są w elastycznych strukturach przypominających format JSON\footnote{JSON - z jęz. ang. JavaScript Object Notation, tekstowy format zapisu i wymiany danych oparty na strukturze obiektów oraz tablic, wykorzystywany w komunikacji między aplikacjami.}. Oznacza to brak sztywno zdefiniowanego schematu tabel oraz większą swobodę w organizacji informacji.
...
\textbf{Cel:} Szybki dostęp do śledzonych tasków, notatek, checklist i projektów z poziomu strony głównej
...
\textbf{2) Utworzenie nowego taska z parametrami}

\textbf{Aktor:} Pracownik biurowy

\textbf{Cel:} Uporządkowanie pracy poprzez utworzenie taska z właściwościami
...
\textbf{Warunki końcowe:} Dodany task wraz z opisem, priorytetem i datą realizacji pojawia się na liście tasków.
...
\textbf{3) Utworzenie checklisty zakupów z możliwością odhaczania pozycji}
...
\textbf{5) Utworzenie projektu i zarządzanie taskami w widoku tablicy}

\textbf{Aktor:} Student

\textbf{Cel:} Organizacja pisania pracy dyplomowej poprzez projekt i powiązane taski
...
\FirstParagraph W niniejszym rozdziale opisano wybrane elementy implementacji warstwy frontendowej aplikacji. Skupiono się na częściach systemu, które mają szczególne znaczenie dla ergonomii pracy użytkownika: widoku kanban w projekcie, sekcji przypiętych elementów na dashboardzie oraz formularzach odpowiedzialnych za tworzenie i edycję tasków oraz projektów. Opisane fragmenty zostały zaimplementowane jako komponenty React, działające w środowisku Next.js i komunikujące się z backendem za pomocą endpointów API.

\section{Wspólne komponenty interfejsu}

\FirstParagraph Warstwa frontendowa aplikacji została oparta na zestawie wspólnych komponentów, które porządkują wygląd i zachowanie powtarzalnych części interfejsu. Dzięki temu formularze, karty elementów, sekcje listujące dane oraz przyciski akcji zachowują spójny układ wizualny i podobny sposób działania w różnych modułach aplikacji.
...
Wspólne komponenty wykorzystano również przy obsłudze tagów i wyboru powiązanych elementów. Edytory tagów pozwalają dodawać i usuwać etykiety w podobny sposób w formularzach tasków oraz projektów, natomiast komponenty wyboru checklist i notatek pomagają utrzymać jednolity sposób łączenia danych. Takie podejście ogranicza powielanie kodu, ułatwia późniejsze zmiany w interfejsie i zwiększa przewidywalność działania aplikacji dla użytkownika.

\section{Implementacja widoku kanban}

\FirstParagraph Widok kanban został zaimplementowany w komponencie ``\texttt{ProjectKanbanBoard}'', znajdującym się w pliku ``\texttt{project-kanban-board.tsx}''. Komponent ten odpowiada za wyświetlanie tablicy projektu, kolumn statusów oraz kart zadań. Dane wejściowe przekazywane do komponentu obejmują identyfikator projektu, listę kolumn oraz listę zadań. Dzięki temu komponent nie pobiera danych samodzielnie, lecz otrzymuje je z warstwy strony projektu, co upraszcza jego odpowiedzialność i pozwala wykorzystać renderowanie po stronie serwera dla początkowego stanu widoku strony.
...
Lista kolumn jest wyliczana w funkcji ``\texttt{useMemo}''. Jeżeli projekt nie posiada zdefiniowanych kolumn, komponent tworzy kolumnę domyślną ``\texttt{todo}''. Dodatkowo sprawdzane są statusy istniejących zadań. Jeżeli zadanie posiada status, którego nie ma w konfiguracji projektu, tworzona jest dodatkowa kolumna techniczna. Takie rozwiązanie zabezpiecza interfejs przed sytuacją, w której zadanie miałoby zniknąć z tablicy tylko dlatego, że jego status nie występuje już w konfiguracji kolumn.
...
Na powyższym fragmencie widoczne jest zabezpieczenie przed pustą konfiguracją kolumn oraz przed niespójnością między statusem taska a definicją tablicy. Komponent najpierw ustala bazową listę kolumn, a następnie dopisuje kolumny techniczne wynikające z danych tasków. Dzięki temu interfejs zachowuje kompletność nawet wtedy, gdy dane projektu nie są idealnie zsynchronizowane.
...
\FirstParagraph Formularz taska został zaimplementowany w komponencie ``\texttt{TaskForm}'', znajdującym się w pliku ``\texttt{task-form.tsx}''. Komponent obsługuje zarówno tworzenie nowego taska, jak i edycję istniejącego, ponieważ tryb działania jest przekazywany w parametrze ``\texttt{mode}''. Dzięki temu ten sam formularz może zostać użyty w dwóch widokach, bez powielania logiki odpowiedzialnej za pola, walidację podstawową oraz wysyłanie danych do API.
...
\FirstParagraph Formularz projektu został zaimplementowany w komponencie ``\texttt{ProjectForm}'', znajdującym się w pliku ``\texttt{project-form.tsx}''. Podobnie jak formularz taska, komponent obsługuje dwa tryby działania: tworzenie i edycję. Formularz projektu jest jednak bardziej rozbudowany, ponieważ poza podstawowymi polami projektu obsługuje także konfigurację kolumn kanban oraz możliwość dodania początkowych tasków.
...
Rysunek przedstawia pierwszą część logiki odpowiedzialnej za przygotowanie kolumn kanban przed zapisem projektu. Funkcja tworząca identyfikator techniczny ujednolica nazwy kolumn, natomiast normalizacja porządkuje ich listę, usuwa puste wartości i zabezpiecza formularz przed zapisaniem niespójnej konfiguracji.
...
\FirstParagraph W niniejszym rozdziale opisano wybrane, reprezentatywne elementy implementacji backendu aplikacji. Skupiono się na trzech obszarach istotnych dla działania systemu: obsłudze checklist przez API, wspólnych mechanizmach obsługi żądań oraz mechanizmie autoryzacji użytkownika. Fragmenty te są wykonywane po stronie serwera, dlatego odpowiadają za sprawdzanie dostępu do danych, walidację żądań oraz komunikację z bazą danych. Rozdział nie omawia każdego endpointu aplikacji osobno, lecz przedstawia rozwiązania pokazujące najważniejsze wzorce użyte również w pozostałych częściach backendu.
...
\FirstParagraph W backendzie aplikacji wydzielono zestaw funkcji pomocniczych, które są wykorzystywane przez wiele endpointów API. Ich zadaniem jest ograniczenie powtarzania tego samego kodu oraz ujednolicenie sposobu obsługi żądań, odpowiedzi błędów, danych użytkownika i zapisywanych modyfikacji. Dzięki temu handlery poszczególnych zasobów mogą skupiać się na logice domenowej, a wspólne czynności techniczne pozostają zapisane w jednym miejscu.

Pierwszym takim elementem jest moduł ``\texttt{api-request.ts}'', w którym umieszczono funkcje ``\texttt{readJsonBody}'' oraz ``\texttt{parseJsonBody}''. Funkcja ``\texttt{readJsonBody}'' próbuje odczytać treść żądania jako obiekt JSON. Jeżeli ciało żądania jest niepoprawne albo nie ma postaci obiektu, zwracana jest wartość pusta. Funkcja ``\texttt{parseJsonBody}'' rozszerza ten mechanizm o walidację przy użyciu przekazanego schematu Zod. W przypadku błędu zwracana jest para wartości zawierająca ``\texttt{data: null}'' oraz komunikat ``\texttt{error}'', natomiast poprawne dane trafiają do handlera już po przejściu walidacji.
...
Ostatnim przykładem wspólnego mechanizmu jest funkcja ``\texttt{recordActivityEvent}'', zapisana w module ``\texttt{activity-events.ts}''. Funkcja przyjmuje identyfikator użytkownika, typ encji, identyfikator encji, nazwę akcji oraz opcjonalne metadane, a następnie tworzy dokument w kolekcji zdarzeń aktywności. Dzięki temu różne części backendu mogą zapisywać historię operacji w jednolitym formacie. W aplikacji jest to wykorzystywane między innymi przy tworzeniu, aktualizowaniu, przenoszeniu i archiwizowaniu elementów.
...
W konfiguracji zastosowano strategię ``\texttt{jwt}'' oraz callback ``\texttt{session}''\footnote{Auth.js, \emph{Auth.js Core Reference}, dokumentacja callbacków biblioteki Auth.js, [b.r.], dostęp: \url{https://authjs.dev/reference/core\#callbacks} (16.08.2026).}. Jak opisano w podrozdziale 3.5, sesja służy do rozpoznania zalogowanego użytkownika; w implementacji callback dopisuje identyfikator użytkownika do obiektu ``\texttt{session.user.id}'', aby pozostałe części backendu mogły korzystać z jednolitego sposobu identyfikacji właściciela danych.
...
W aplikacji testy zostały uruchamiane za pomocą narzędzia Vitest, które jest dostosowane do testowania kodu JavaScript i TypeScript. Dokumentacja Vitest wskazuje, że test opisuje oczekiwane zachowanie wybranego fragmentu kodu, a funkcje ``\texttt{test}'' oraz ``\texttt{expect}'' pozwalają definiować sprawdzane przypadki i porównywać wynik działania programu z oczekiwanym rezultatem\footnote{Vitest, \emph{Writing Tests}, dokumentacja pisania testów w Vitest, [b.r.], dostęp: \url{https://vitest.dev/guide/learn/writing-tests.html} (03.08.2026).}. W kontekście aplikacji React podobną rolę pełnią narzędzia pozwalające renderować komponenty w środowisku testowym i sprawdzać widoczne dla użytkownika elementy interfejsu; dokumentacja React Testing Library opisuje m.in. funkcję ``\texttt{render}'', służącą do osadzania komponentu w kontenerze testowym\footnote{Testing Library, \emph{React Testing Library API}, dokumentacja API biblioteki React Testing Library, [b.r.], dostęp: \url{https://testing-library.com/docs/react-testing-library/api/} (03.08.2026).}.
...
\FirstParagraph Celem pracy było zaprojektowanie oraz zaimplementowanie aplikacji internetowej wspierającej indywidualną organizację zadań, checklist, notatek i projektów. W toku pracy omówiono potrzeby użytkownika indywidualnego, porównano wybrane aplikacje o podobnym przeznaczeniu, przedstawiono zastosowane technologie frontendowe i backendowe oraz opisano architekturę systemu. Następnie zaprezentowano projekt interfejsu użytkownika, scenariusze przypadków użycia, strukturę bazy danych, wybrane fragmenty implementacji oraz testy jednostkowe dotyczące funkcjonalności tasków. Założony cel pracy został osiągnięty, ponieważ przygotowano działającą aplikację pozwalającą użytkownikowi organizować dane w kilku powiązanych modułach i korzystać z nich z poziomu przeglądarki internetowej.

W wyniku wykonanych prac powstała funkcjonalna aplikacja klient-serwer, oparta na środowisku Next.js, bibliotece React, języku TypeScript oraz bazie danych MongoDB. System umożliwia tworzenie i organizowanie tasków, checklist, notatek oraz projektów, a także prezentowanie projektów w widoku listy lub tablicy kanban. Zaimplementowano również mechanizm przypinania najważniejszych elementów do dashboardu, obsługę logowania z wykorzystaniem zewnętrznych dostawców OAuth, walidację danych wejściowych oraz podstawowe testy zabezpieczające logikę związaną z taskami.

Przyjęta architektura pozwoliła oddzielić warstwę interfejsu użytkownika od logiki serwerowej i warstwy przechowywania danych. Endpointy API pełnią rolę pośrednika między frontendem a bazą danych, dzięki czemu możliwe jest sprawdzanie sesji użytkownika, ograniczanie dostępu do cudzych zasobów oraz walidowanie danych przed wykonaniem operacji zapisu. Dokumentowy model MongoDB okazał się dopasowany do charakteru aplikacji, ponieważ ułatwia przechowywanie elementów o elastycznej strukturze, takich jak projekty z własnymi kolumnami kanban, tagami i powiązanymi checklistami.

Samodzielny wkład autora obejmował analizę wymagań, przygotowanie koncepcji aplikacji, zaprojektowanie struktury bazy danych, wykonanie interfejsu użytkownika, implementację logiki frontendowej i backendowej, konfigurację autoryzacji, przygotowanie scenariuszy przypadków użycia oraz opracowanie testów jednostkowych dla funkcjonalności tasków. Autor przygotował również opis architektury, diagramy, dokumentację implementacji oraz materiały graficzne wykorzystane w pracy.

Projekt może być dalej rozwijany między innymi przez rozbudowę testów integracyjnych, dodanie powiadomień, udoskonalenie wyszukiwania, rozwinięcie statystyk produktywności oraz przygotowanie wersji wdrożeniowej aplikacji. Możliwe byłoby także rozszerzenie systemu o współdzielenie projektów między użytkownikami, jednak taka funkcja wymagałaby dodatkowego modelu uprawnień oraz bardziej rozbudowanych mechanizmów kontroli dostępu. Dalszy rozwój mógłby obejmować również eksport danych, cykliczne taski, zaawansowane filtry oraz synchronizację z zewnętrznymi kalendarzami. Obecna wersja spełnia jednak główne założenie pracy, którym było przygotowanie przejrzystego narzędzia do indywidualnego zarządzania zadaniami i projektami.
...
\item {\RaggedRight Ahmad M. O., Dennehy D., Conboy K., Oivo M., \emph{Kanban in Software Engineering: A Systematic Mapping Study}, ``Journal of Systems and Software'' 2018, Vol. 137, s. 96--113, DOI: 10.1016/j.jss.2017.11.045, dostęp: \url{https://doi.org/10.1016/j.jss.2017.11.045} (31.07.2026).\par}
