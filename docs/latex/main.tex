...
\onehalfspacing
\AtBeginDocument{\setlength{\parindent}{0pt}\justifying}
\frenchspacing
\newlength{\firstparagraphindent}
\setlength{\firstparagraphindent}{1.25cm}
\setlength{\parindent}{0pt}
\setlength{\parskip}{0pt}
\newcommand{\FirstParagraph}{\noindent\hspace*{\firstparagraphindent}\ignorespaces}
...
\renewcommand{\footnotelayout}{\RaggedRight\setstretch{1}\fontsize{10}{12}\selectfont\setlength{\parindent}{0pt}}
...
Inspiracją dla tematu są popularne aplikacje do zarządzania projektami, kładące duży nacisk na organizację pracy zespołowej. Narzędzia takie jak Trello\footnote{Por. [3]: Atlassian, \emph{Trello},\newline \url{https://trello.com} (dostęp: 17.07.2026).}, Asana\footnote{Por. [2]: Asana, Inc., \emph{Asana},\newline \url{https://asana.com} (dostęp: 17.07.2026).} czy ClickUp\footnote{Por. [6]: ClickUp, \emph{ClickUp},\newline \url{https://clickup.com} (dostęp: 17.07.2026).} dostarczają szeroką gamę funkcji, umożliwiających organizację zadań, jednak wiele z nich jest niepotrzebne użytkownikowi indywidualnemu. Z kolei nadmiar zbędnych funkcji może utrudniać użytkownikowi korzystanie z aplikacji. Dlatego też
...
Rozdział ósmy przedstawia wybrane elementy implementacji backendu. Opisano w nim handler API odpowiedzialny za obsługę checklist oraz konfigurację mechanizmu autoryzacji, który pozwala identyfikować użytkownika i ograniczać dostęp do jego danych.

Rozdział dziewiąty dotyczy testów jednostkowych. Wyjaśniono w nim rolę testów w projekcie oraz opisano przygotowane testy walidacji danych tasków, które sprawdzają poprawność danych wejściowych przed wykonaniem operacji zapisu lub aktualizacji.
...
\FirstParagraph W procesie projektowania aplikacji internetowej istotne znaczenie ma dokładne zrozumienie potrzeb przyszłych użytkowników oraz sposobu, w jaki będą oni korzystać
...
Fundamentalne narzędzia używane do tworzenia frontendu to HTML\footnote{HTML - z jęz. ang. Hypertext Markup Language, czyli ``hipertekstowy język znaczników''. Por. [7]: Dean J., \emph{Web programming with HTML, CSS, and JavaScript}, Burlington 2017.}, CSS\footnote{CSS - z jęz. ang. Cascading Style Sheets, oznaczające ``kaskadujące arkusze stylów''. Por. [7]: Dean J., \emph{Web programming with HTML, CSS, and JavaScript}, Burlington 2017.} oraz JavaScript\footnote{JavaScript - język programowania wykorzystywany do tworzenia interaktywnych elementów stron internetowych. Por. [7]: Dean J., \emph{Web programming with HTML, CSS, and JavaScript}, Burlington 2017.}. HTML to język znaczników, odpowiadający za strukturę strony internetowej. Określa on semantyczny układ treści, takich jak nagłówki, akapity, listy, obrazy, czy formularze. HTML stanowi szkielet warstwy frontendowej aplikacji webowej, będąc interpretowanym przez przeglądarkę w celu prezentacji treści użytkownikowi. Poprawne i semantyczne rozmieszczenie znaczników HTML ma kluczowe znaczenie dla dostępności cyfrowej, ponieważ umożliwia prawidłową interpretację treści przez czytniki ekranu wykorzystywane przez osoby
...
Przykładem jest tutaj React\footnote{Por. [12]: React, \emph{React the library for web and native interfaces},\newline \url{https://react.dev} (dostęp: 28.01.2026).}, czyli biblioteka języka JavaScript.
...
\FirstParagraph Komponenty w React stanowią podstawowy element budowy interfejsu użytkownika
...
\FirstParagraph TailwindCSS\footnote{Por. [13]: Tailwind Labs, \emph{Tailwind CSS},\newline \url{https://tailwindcss.com} (dostęp: 28.01.2026).} jest frameworkiem służącym do stylowania interfejsów użytkownika za pomocą gotowych klas CSS, używanych bezpośrednio
...
\FirstParagraph Routing w Next.js stanowi mechanizm odpowiedzialny za mapowanie adresów URL na konkretne widoki aplikacji. W przeciwieństwie do klasycznych aplikacji React, w których konfiguracja tras wymaga dodatkowych bibliotek i ręcznego definiowania reguł nawigacji, Next.js wykorzystuje routing oparty na strukturze plików - z jęz. ang. file-based routing. Ścieżki URL są automatycznie generowane na podstawie organizacji katalogów i plików
...
\FirstParagraph W Next.js autentykację i autoryzację można w prosty sposób wdrożyć poprzez integrację
...
Rozwiązanie to ogranicza potrzebę utrzymywania osobnego projektu backendowego i ułatwia rozwijanie aplikacji jako jednej spójnej całości. Dodatkową zaletą jest wsparcie dla SEO, w tym lepszej indeksowalności strony\footnote{Indeksowalność strony - cecha strony internetowej oznaczająca, że jej treść może zostać poprawnie odczytana, zinterpretowana i umieszczona w indeksie wyszukiwarki internetowej.} dzięki renderowaniu treści po stronie serwera oraz możliwości zarządzania metadanymi dokumentu. Istotne było także proste połączenie z Auth.js\footnote{Por. [4]: Auth.js, \emph{Auth.js},\newline \url{https://authjs.dev} (dostęp: 17.07.2026).}, które pozwala zabezpieczać widoki i endpointy bez tworzenia całego mechanizmu logowania od podstaw.
...
\begin{figure}[H]
  ...
  \caption[Przedstawienie procesu wyświetlania danych użytkownikowi aplikacji internetowej]{Przedstawienie procesu, umożliwiającego wyświetlenie danych użytkownikowi aplikacji internetowej. \newline Źródło: [8], \url{https://d1zruf9db62p8s.cloudfront.net/2025/08/Frontend-vs-Backend.webp} (dostęp: 02.02.2026).}
\end{figure}

\section{MySQL}

\FirstParagraph MySQL\footnote{Por. [11]: Oracle, \emph{MySQL Documentation},\newline \url{https://dev.mysql.com/doc/} (dostęp: 25.07.2026).} należy do najczęściej wykorzystywanych technologii bazodanowych w aplikacjach internetowych ze względu na wysoką wydajność, stabilność, łatwość konfiguracji oraz szerokie wsparcie społeczności i narzędzi programistycznych. Ten typ bazy danych wykorzystuje język SQL do definiowania struktury danych, wykonywania zapytań oraz zarządzania integralnością informacji.
...
\FirstParagraph MongoDB\footnote{Por. [10]: MongoDB, Inc., \emph{Welcome to the MongoDB docs},\newline \url{https://www.mongodb.com/docs/} (dostęp: 02.02.2026).} jest nierelacyjnym systemem zarządzania bazą danych, którego podstawową różnicą względem relacyjnych rozwiązań, jest sposób przechowywania informacji. MongoDB wykorzystuje model dokumentowy, w którym dane zapisywane są
...
\FirstParagraph W niniejszej pracy, kluczowe znaczenie ma łatwość rozbudowy funkcjonalności oraz możliwość obsługi dynamicznie zmieniających się struktur informacji. Z tego względu zdecydowano się na zastosowanie bazy dokumentowej MongoDB zamiast klasycznej relacyjnej bazy danych.
...
\begin{figure}[H]
  ...
  \caption[Diagram warstw aplikacji internetowej]{Diagram, przedstawiający warstwy aplikacji internetowej. \newline Źródło: [1], \url{https://www.albiorixtech.com/wp-content/uploads/2025/12/web-application-architecture-layers.jpg} (dostęp: 03.02.2026).}
\end{figure}
...
Kolejne diagramy, powstałe na etapie projektowania architektury przygotowano, wykorzystując informacje z artykułu ``Canva: DFD''\footnote{Por. [5]: Canva, \emph{Creating a data flow diagram: How-tos, templates, and tips},\newline \url{https://www.canva.com/online-whiteboard/data-flow-diagrams/} (dostęp: 03.02.2026).}. Pokazują jak dane przemieszczają się w systemie między źródłami, procesami oraz magazynami danych. Są to tak zwane jest ``diagramy przepływu danych''.
...
\chapter{Implementacja backendu}
...
\FirstParagraph Obsługa checklist została zaimplementowana w routach \texttt{/api/checklists} oraz \texttt{/api/checklists/\{checklistId\}}. Pierwszy route odpowiada za operacje wykonywane na całej kolekcji checklist, natomiast drugi za operacje dotyczące pojedynczej checklisty wskazanej przez identyfikator w adresie żądania. Takie rozdzielenie wynika ze sposobu działania mechanizmu routingu w Next.js, w którym struktura katalogów określa ścieżki dostępne w API.
...
Do przechowywania danych autoryzacyjnych wykorzystano adapter \texttt{MongoDBAdapter}. Adapter zapisuje w bazie informacje o użytkownikach i powiązanych kontach zewnętrznych, dzięki czemu aplikacja może rozpoznawać tę samą osobę po kolejnych logowaniach. Dane te są przechowywane w kolekcjach technicznych tworzonych przez mechanizm NextAuth, niezależnie od domenowych kolekcji aplikacji, takich jak taski, checklisty czy projekty.

Sesja użytkownika działa w strategii \texttt{jwt}. JWT\footnote{JWT - skrót od JSON Web Token; jest to format tokenu używany do przenoszenia informacji o użytkowniku lub uprawnieniach między stronami komunikacji.} pozwala przechowywać podstawowe informacje o zalogowanym użytkowniku w tokenie sesyjnym. W konfiguracji zastosowano callback\footnote{Callback - funkcja przekazywana do innego mechanizmu, która jest wywoływana automatycznie w określonym momencie działania programu.} \texttt{session}, który przenosi identyfikator użytkownika z tokenu do obiektu \texttt{session.user.id}. Dzięki temu pozostałe części backendu mogą korzystać z jednolitego sposobu identyfikacji właściciela danych.

W praktyce autoryzacja jest używana w handlerach API przez funkcję \texttt{getCurrentUserId}. Każda operacja na danych użytkownika rozpoczyna się od pobrania identyfikatora sesji. Następnie identyfikator ten trafia do warunków zapytań jako \texttt{ownerId}. Takie rozwiązanie sprawia, że nawet poprawnie zbudowane żądanie nie pozwala odczytać ani zmienić danych należących do innej osoby.

\chapter{Testy jednostkowe}

\FirstParagraph Testy jednostkowe\footnote{Test jednostkowy - test sprawdzający niewielki, wydzielony fragment programu, np. funkcję lub schemat walidacji, bez uruchamiania całego systemu.} służą do sprawdzania małych fragmentów aplikacji w kontrolowanych warunkach. Ich zadaniem jest szybkie wykrywanie błędów w logice programu oraz zabezpieczenie już działających funkcji przed przypadkowymi regresjami podczas dalszego rozwoju projektu. W aplikacji testy zostały uruchamiane za pomocą narzędzia Vitest, które jest dostosowane do testowania kodu JavaScript i TypeScript.
...
W testach wykorzystano asercje\footnote{Asercja - warunek sprawdzany podczas testu, którego spełnienie oznacza poprawne zachowanie badanego fragmentu programu.} porównujące oczekiwany wynik walidacji z wynikiem rzeczywistym. Dzięki temu możliwe jest szybkie sprawdzenie, czy zmiany w schematach danych nie osłabiły reguł zabezpieczających API tasków.
...
\item {\RaggedRight Albiorix Technology, \emph{Web application architecture layers},\newline \url{https://www.albiorixtech.com/wp-content/uploads/2025/12/web-application-architecture-layers.jpg} (dostęp: 03.02.2026).\par}

\item {\RaggedRight Asana, Inc., \emph{Asana},\newline \url{https://asana.com} (dostęp: 17.07.2026).\par}

\item {\RaggedRight Atlassian, \emph{Trello},\newline \url{https://trello.com} (dostęp: 17.07.2026).\par}

\item {\RaggedRight Auth.js, \emph{Auth.js},\newline \url{https://authjs.dev} (dostęp: 17.07.2026).\par}

\item {\RaggedRight Canva, \emph{Creating a data flow diagram: How-tos, templates, and tips},\newline \url{https://www.canva.com/online-whiteboard/data-flow-diagrams/} (dostęp: 03.02.2026).\par}

\item {\RaggedRight ClickUp, \emph{ClickUp},\newline \url{https://clickup.com} (dostęp: 17.07.2026).\par}

\item Dean J., \emph{Web programming with HTML, CSS, and JavaScript}, Burlington 2017.

\item {\RaggedRight \emph{Frontend vs Backend},\newline \url{https://d1zruf9db62p8s.cloudfront.net/2025/08/Frontend-vs-Backend.webp} (dostęp: 02.02.2026).\par}

\item {\RaggedRight Microsoft, \emph{TypeScript Documentation},\newline \url{https://www.typescriptlang.org/docs/} (dostęp: 25.07.2026).\par}

\item {\RaggedRight MongoDB, Inc., \emph{Welcome to the MongoDB docs},\newline \url{https://www.mongodb.com/docs/} (dostęp: 02.02.2026).\par}

\item {\RaggedRight Oracle, \emph{MySQL Documentation},\newline \url{https://dev.mysql.com/doc/} (dostęp: 25.07.2026).\par}

\item {\RaggedRight React, \emph{React: The library for web and native interfaces},\newline \url{https://react.dev} (dostęp: 28.01.2026).\par}

\item {\RaggedRight Tailwind Labs, \emph{Tailwind CSS},\newline \url{https://tailwindcss.com} (dostęp: 28.01.2026).\par}

\item {\RaggedRight Vercel, \emph{Next.js Docs},\newline \url{https://nextjs.org/docs} (dostęp: 29.01.2026).\par}
