...
Inspiracją dla tematu są popularne aplikacje do zarządzania projektami, kładące duży nacisk na organizację pracy zespołowej. Narzędzia takie jak Trello\footnote{Por. [1].}, Asana\footnote{Por. [2].} czy ClickUp\footnote{Por. [3].} dostarczają szeroką gamę funkcji, umożliwiających organizację zadań, jednak wiele z nich jest niepotrzebne użytkownikowi indywidualnemu. Z kolei nadmiar zbędnych funkcji może utrudniać użytkownikowi korzystanie z aplikacji. Dlatego też
...
Fundamentalne narzędzia używane do tworzenia frontendu to HTML\footnote{HTML - z jęz. ang. Hypertext Markup Language, czyli ``hipertekstowy język znaczników''. Por. [4].}, CSS\footnote{CSS - z jęz. ang. Cascading Style Sheets, oznaczające ``kaskadujące arkusze stylów''. Por. [4].} oraz JavaScript\footnote{JavaScript - język programowania wykorzystywany do tworzenia interaktywnych elementów stron internetowych. Por. [4].}. HTML to język znaczników, odpowiadający za strukturę strony internetowej. Określa on semantyczny układ treści, takich jak nagłówki, akapity, listy, obrazy czy formularze. HTML stanowi szkielet warstwy frontendowej aplikacji webowej, będąc interpretowanym przez przeglądarkę w celu prezentacji treści użytkownikowi. Odpowiednie ustawienie Poprawne i semantyczne rozmieszczenie znaczników HTML ma kluczowe znaczenie dla dostępności cyfrowej, ponieważ umożliwia prawidłową interpretację treści przez czytniki ekranu wykorzystywane przez osoby
...
Najważniejszym przykładem jest tutaj React\footnote{Por. [5]: \emph{React the library for web and native interfaces}, \url{https://react.dev} (dostęp: 28.01.2026).}, czyli biblioteka języka JavaScript.
...
Istotną zaletą Nexta jest również integracja z biblioteką Auth.js\footnote{Por. [8].}. Pozwala to implementować zabezpieczenia zarówno po stronie klienta - jak warunkowe renderowanie widoków, oraz po stronie serwera - ochrona endpointów API, czy walidacja sesji
...
\begin{figure}[H]
  ...
  \caption[Przedstawienie procesu wyświetlania danych użytkownikowi aplikacji internetowej]{Przedstawienie procesu, umożliwiającego wyświetlenie danych użytkownikowi aplikacji internetowej. Źródło: [9].}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[Diagram warstw aplikacji internetowej]{Diagram, przedstawiający warstwy aplikacji internetowej. Źródło: [11].}
\end{figure}
...
Na rysunku \ref{fig:schemat-bazy-danych-aplikacji} przedstawiono graficzny schemat struktury bazy danych aplikacji. Diagram został podzielony na cztery grupy: kolekcje autoryzacyjne, kolekcje przechowujące dane użytkownika, kolekcje pomocnicze oraz pola osadzone. Taki podział pokazuje, że dane odpowiedzialne za logowanie użytkownika są odseparowane od danych domenowych aplikacji, takich jak zadania, projekty, checklisty i notatki.
...
Kolekcja \texttt{projects} jest jedną z najważniejszych kolekcji domenowych przedstawionych na schemacie, ponieważ łączy informacje opisowe projektu z jego strukturą roboczą. Oprócz podstawowych pól, takich jak \texttt{title}, \texttt{priority} i \texttt{dueDate}, dokument projektu przechowuje identyfikatory powiązanych tasków i checklist oraz osadzoną tablicę \texttt{kanbanColumns}. Dzięki temu projekt zawiera nie tylko dane opisowe, ale również konfigurację sposobu prezentacji pracy użytkownikowi.
...
Rysunki \ref{fig:scenariusz-1-checklista-przypiecie}--\ref{fig:scenariusz-1-przypiete-elementy} przedstawiają przebieg scenariusza przypięcia checklisty do ekranu głównego. Widok szczegółów checklisty został zaprojektowany jako centralny panel z wyraźnym tytułem, ikoną typu obiektu oraz listą pozycji oznaczanych kwadratowymi polami wyboru. Wykonane elementy checklisty są odróżnione kolorem akcentu i przekreśleniem tekstu, dzięki czemu użytkownik może szybko ocenić postęp realizacji. Przycisk przypięcia został umieszczony przy prawej krawędzi panelu, w pobliżu pozostałych akcji na obiekcie. Po przypięciu zmienia się jego wypełnienie oraz kolorystyka, co daje czytelną informację zwrotną bez konieczności opuszczania aktualnego ekranu. Na dashboardzie przypięte elementy są prezentowane jako karty z ikonami typów, tytułami, tagami i krótkim podglądem zawartości, a ich rozmieszczenie w siatce pozwala skanować najważniejsze obiekty bez przechodzenia do osobnych modułów aplikacji.
...
Rysunki \ref{fig:scenariusz-2-taski}--\ref{fig:scenariusz-2-formularz-taska} odnoszą się do scenariusza utworzenia taska. Lista tasków wykorzystuje stały układ aplikacji z bocznym menu nawigacyjnym i nagłówkiem sekcji, dzięki czemu użytkownik od razu widzi, w którym module pracuje. Nad kartami z taskami umieszczono pasek filtrów, sortowania i wyszukiwania, a przycisk utworzenia nowego taska wyróżniono kolorem akcentu w prawym górnym rogu widoku. Same taski zostały przedstawione w formie kart zawierających nazwę, status, priorytet, tagi oraz krótki opis. Formularz tworzenia taska zachowuje tę samą hierarchię wizualną: najpierw eksponowane są pola tytułu i opisu, następnie parametry organizacyjne, takie jak priorytet, status, projekt i termin realizacji. Niżej znajdują się tagi, powiązane checklisty oraz notatki, co porządkuje dane od najważniejszych informacji opisowych do relacji z innymi elementami systemu.
...
Rysunki \ref{fig:scenariusz-5-formularz-projektu-pierwsza-czesc}--\ref{fig:scenariusz-5-lista-taskow-projektu} prezentują interfejs związany ze scenariuszem utworzenia projektu i zarządzania podzadaniami. Formularz projektu został podzielony na logiczne części, co ogranicza wizualne przeciążenie mimo dużej liczby parametrów. W pierwszej części użytkownik uzupełnia podstawowe dane, takie jak tytuł, opis, priorytet, status, termin, tagi oraz powiązane checklisty i notatki. Druga część formularza obejmuje konfigurację kolumn kanban, w której każda kolumna jest pokazana jako osobny wiersz z nazwą, kolorem, oznaczeniem zakończenia oraz przyciskami zmiany kolejności i usunięcia. Sekcja dodawania tasków do projektu wykorzystuje układ tabelaryczny, dzięki czemu kilka podzadań można porównać w jednym poziomym rzędzie według tytułu, priorytetu, statusu i terminu.

Po utworzeniu projektu użytkownik może wybrać jeden z dwóch sposobów przeglądania tasków. Widok kanban eksponuje kolumny statusów jako równoległe obszary robocze, z kolorowymi oznaczeniami nagłówków i paskami postępu. Karty tasków znajdują się bezpośrednio w kolumnach odpowiadających ich statusom, a przyciski przejścia do poprzedniej lub następnej kolumny wzmacniają czytelność procesu. Alternatywny widok listy przedstawia te same taski w bardziej zwartym układzie, przeznaczonym do szybkiego przeglądania nazw, priorytetów oraz aktualnych statusów. Przełącznik pomiędzy widokiem kanban i listą jest widoczny nad obszarem tasków, dzięki czemu użytkownik może dobrać prezentację danych do aktualnego sposobu pracy.
