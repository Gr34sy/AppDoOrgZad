...
\chapter[Frontend]{Frontend\footnotemark}
\footnotetext{Frontend - termin pochodzący z jęz. ang., będący złożeniem słów ``front'' (przód, część widoczna) oraz ``end'' (część, warstwa), odnoszący się do tej części aplikacji, która jest bezpośrednio dostępna dla użytkownika.}

\FirstParagraph Frontend to część strony internetowej lub aplikacji, z którą użytkownik bezpośrednio wchodzi w interakcję. Obejmuje on wszystkie akcje, wykonujące się po stronie przeglądarki. Na stronie internetowej do frontendu zalicza się:
...
\FirstParagraph Mechanizm hooków\footnote{\emph{Built-in React Hooks}, dokumentacja hooków biblioteki React, [b.r.], \url{https://react.dev/reference/react/hooks} (dostęp: 16.08.2026).} umożliwia przechowywanie danych pomiędzy kolejnymi wywołaniami funkcji komponentu, mimo że sama funkcja jest wykonywana od nowa przy każdym renderowaniu. Każdy hook jest wywoływany bezpośrednio w ciele funkcji komponentu, dzięki czemu React może jednoznacznie przypisać jego działanie do danego komponentu oraz zachować odpowiednie wartości pomiędzy kolejnymi wywołaniami tej funkcji. Kolejność wywołań hooków ma kluczowe znaczenie, ponieważ na jej podstawie React identyfikuje, które dane należą do danego hooka.
...
\begin{figure}[H]
  ...
  \caption{Przykładowe użycie Tailwind CSS, pochodzące z oficjalnej strony frameworka (dostęp: 28.01.2026)}
\end{figure}
...
\chapter[Backend]{Backend\footnotemark}
\footnotetext{Backend - termin pochodzący z jęz. ang., będący złożeniem słów ``back'' (tył, zaplecze) oraz ``end'' (część, warstwa), odnoszący się do części aplikacji niedostępnej dla użytkownika.}
...
W projekcie zdecydowano się na wykorzystanie Next.js, ponieważ framework ogranicza potrzebę utrzymywania osobnego projektu backendowego i ułatwia rozwijanie aplikacji jako jednej spójnej całości. W decyzji wykorzystania Nexta istotna była także możliwość prostego połączenia z biblioteką Auth.js\footnote{Auth.js, \emph{Auth.js}, dokumentacja biblioteki do obsługi uwierzytelniania, [b.r.], \url{https://authjs.dev} (dostęp: 17.07.2026).}, która pozwala zabezpieczać widoki i endpointy bez tworzenia całego mechanizmu logowania od podstaw.
...
\begin{figure}[H]
  ...
  \caption[Diagram warstw aplikacji internetowej]{Diagram, przedstawiający warstwy aplikacji internetowej. Źródło: Albiorix Technology, \url{https://www.albiorixtech.com/wp-content/uploads/2025/12/web-application-architecture-layers.jpg} (dostęp: 03.02.2026)}
\end{figure}
...
\FirstParagraph W aplikacji zastosowano bazę danych MongoDB oraz bibliotekę Mongoose, która została wykorzystana do opisania struktury dokumentów domenowych. Połączenie z bazą jest realizowane przez moduł odpowiedzialny za utrzymywanie współdzielonego połączenia Mongoose, natomiast mechanizm logowania OAuth korzysta dodatkowo z adaptera MongoDB dla NextAuth\footnote{NextAuth to biblioteka wykorzystywana w aplikacjach Next.js do obsługi logowania, sesji użytkownika oraz integracji z zewnętrznymi dostawcami tożsamości. Projekt NextAuth.js jest obecnie rozwijany w ramach szerszego ekosystemu Auth.js. Źródło: Auth.js, \emph{Auth.js Core Reference}, dokumentacja callbacków biblioteki Auth.js, [b.r.], \url{https://authjs.dev/reference/core\#callbacks} (dostęp: 16.08.2026).}. Dzięki temu dane logowania oraz dane aplikacyjne są przechowywane w tej samej bazie, ale obsługiwane przez odrębne warstwy aplikacji.
...
W aplikacji testy zostały uruchamiane za pomocą narzędzia Vitest, które jest dostosowane do testowania kodu JavaScript i TypeScript. Dokumentacja Vitest wskazuje, że test opisuje oczekiwane zachowanie wybranego fragmentu kodu, a funkcje ``\texttt{test}'' oraz ``\texttt{expect}'' pozwalają definiować sprawdzane przypadki i porównywać wynik działania programu z oczekiwanym rezultatem\footnote{Vitest, \emph{Writing Tests}, dokumentacja pisania testów w Vitest, [b.r.], \url{https://vitest.dev/guide/learn/writing-tests.html} (dostęp: 03.08.2026).}. W kontekście aplikacji React podobną rolę pełnią narzędzia pozwalające renderować komponenty w środowisku testowym i sprawdzać widoczne dla użytkownika elementy interfejsu; dokumentacja React Testing Library opisuje m.in. funkcję ``\texttt{render}'', służącą do osadzania komponentu w kontenerze testowym\footnote{Testing Library, \emph{React Testing Library API}, dokumentacja API biblioteki React Testing Library, [b.r.], \url{https://testing-library.com/docs/react-testing-library/api/} (dostęp: 03.08.2026).}.
...
\item {\RaggedRight Ahmad M. O., Dennehy D., Conboy K., Oivo M., \emph{Kanban in Software Engineering: A Systematic Mapping Study}, ``Journal of Systems and Software'' 2018, Vol. 137, s. 96--113, dostęp: \url{https://doi.org/10.1016/j.jss.2017.11.045} (31.07.2026).\par}

\item {\RaggedRight Albiorix Technology, \emph{Web Application Architecture Layers}, grafika przedstawiająca warstwy architektury aplikacji internetowej, [b.r.], dostęp: \url{https://www.albiorixtech.com/wp-content/uploads/2025/12/web-application-architecture-layers.jpg} (03.02.2026).\par}

\item {\RaggedRight Asana, Inc., \emph{Asana}, strona internetowa aplikacji do zarządzania pracą, [b.r.], dostęp: \url{https://asana.com} (17.07.2026).\par}

\item {\RaggedRight Atlassian, \emph{Trello}, strona internetowa aplikacji Kanban do zarządzania zadaniami, [b.r.], dostęp: \url{https://trello.com} (17.07.2026).\par}

\item {\RaggedRight Auth.js, \emph{Auth.js}, dokumentacja biblioteki do obsługi uwierzytelniania, [b.r.], dostęp: \url{https://authjs.dev} (17.07.2026).\par}

\item {\RaggedRight Auth.js, \emph{Auth.js Core Reference}, dokumentacja callbacków biblioteki Auth.js, [b.r.], dostęp: \url{https://authjs.dev/reference/core\#callbacks} (16.08.2026).\par}

\item {\RaggedRight Canva, \emph{Creating a Data Flow Diagram: How-tos, Templates, and Tips}, artykuł dotyczący tworzenia diagramów przepływu danych, [b.r.], dostęp: \url{https://www.canva.com/online-whiteboard/data-flow-diagrams/} (03.02.2026).\par}

\item {\RaggedRight ClickUp, \emph{ClickUp}, strona internetowa aplikacji do zarządzania projektami, [b.r.], dostęp: \url{https://clickup.com} (17.07.2026).\par}

\item {\RaggedRight Dean J., \emph{Web Programming with HTML5, CSS, and JavaScript}, Jones \& Bartlett Learning, Burlington 2017.\par}

\item {\RaggedRight \emph{Frontend vs Backend}, grafika przedstawiająca porównanie warstwy frontendowej i backendowej, [b.r.], dostęp: \url{https://d1zruf9db62p8s.cloudfront.net/2025/08/Frontend-vs-Backend.webp} (02.02.2026).\par}

\item {\RaggedRight Microsoft, \emph{TypeScript Documentation}, dokumentacja języka TypeScript, [b.r.], dostęp: \url{https://www.typescriptlang.org/docs/} (25.07.2026).\par}

\item {\RaggedRight MongoDB, Inc., \emph{Welcome to the MongoDB Docs}, dokumentacja systemu MongoDB, [b.r.], dostęp: \url{https://www.mongodb.com/docs/} (02.02.2026).\par}

\item {\RaggedRight Mongoose, \emph{Schemas}, dokumentacja biblioteki Mongoose dotycząca schematów danych, [b.r.], dostęp: \url{https://mongoosejs.com/docs/guide.html} (16.08.2026).\par}

\item {\RaggedRight Oracle, \emph{MySQL Documentation}, dokumentacja systemu zarządzania bazą danych MySQL, [b.r.], dostęp: \url{https://dev.mysql.com/doc/} (25.07.2026).\par}

\item {\RaggedRight React, \emph{Built-in React Hooks}, dokumentacja hooków biblioteki React, [b.r.], dostęp: \url{https://react.dev/reference/react/hooks} (16.08.2026).\par}

\item {\RaggedRight React, \emph{React: The Library for Web and Native Interfaces}, dokumentacja biblioteki React, [b.r.], dostęp: \url{https://react.dev} (28.01.2026).\par}

\item {\RaggedRight Sun S.-T., Beznosov K., \emph{The Devil is in the (Implementation) Details: An Empirical Analysis of OAuth SSO Systems}, [w:] \emph{Proceedings of the 2012 ACM Conference on Computer and Communications Security}, 2012, s. 378--390, DOI: 10.1145/2382196.2382238, dostęp: \url{https://doi.org/10.1145/2382196.2382238} (31.07.2026).\par}

\item {\RaggedRight Tailwind Labs, \emph{Tailwind CSS}, dokumentacja frameworka CSS, [b.r.], dostęp: \url{https://tailwindcss.com} (28.01.2026).\par}

\item {\RaggedRight Testing Library, \emph{React Testing Library API}, dokumentacja API biblioteki React Testing Library, [b.r.], dostęp: \url{https://testing-library.com/docs/react-testing-library/api/} (03.08.2026).\par}

\item {\RaggedRight Vercel, \emph{Next.js Docs}, dokumentacja frameworka Next.js, [b.r.], dostęp: \url{https://nextjs.org/docs} (29.01.2026).\par}

\item {\RaggedRight Vercel, \emph{Route Handlers}, dokumentacja Next.js dotycząca obsługi endpointów, [b.r.], dostęp: \url{https://nextjs.org/docs/app/getting-started/route-handlers} (16.08.2026).\par}

\item {\RaggedRight Vitest, \emph{Writing Tests}, dokumentacja pisania testów w Vitest, [b.r.], dostęp: \url{https://vitest.dev/guide/learn/writing-tests.html} (03.08.2026).\par}

\item {\RaggedRight Zod, \emph{Defining schemas}, dokumentacja biblioteki Zod dotycząca definiowania schematów walidacji, [b.r.], dostęp: \url{https://zod.dev/api?id=property} (16.08.2026).\par}
