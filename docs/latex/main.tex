...
\FirstParagraph Kolejną technologią zastosowaną w projekcie jest TypeScript\footnote{Microsoft, \emph{TypeScript Documentation}, dokumentacja języka TypeScript, [b.r.], dostęp: \url{https://www.typescriptlang.org/docs/} (25.07.2026).}, czyli nadzbiór języka JavaScript. Rozszerza on JSa o system typów, interfejsy oraz mechanizmy kontroli poprawności kodu na etapie kompilacji. Kod TypeScript nie jest bezpośrednio interpretowany przez przeglądarkę, lecz przed uruchomieniem musi zostać transpilowany do standardowego JavaScriptu, zachowując pełną kompatybilność ze środowiskiem wykonawczym.
...
Na powyższym fragmencie widoczne jest zabezpieczenie przed pustą konfiguracją kolumn oraz przed niespójnością między statusem zadania a definicją tablicy. Komponent najpierw ustala bazową listę kolumn, a następnie dopisuje kolumny techniczne wynikające z danych tasków. Dzięki temu interfejs zachowuje kompletność nawet wtedy, gdy dane projektu nie są idealnie zsynchronizowane.
...
Na fragmencie kodu widać wspólny schemat obsługi żądań listowych. Obie metody najpierw pobierają identyfikator użytkownika z sesji, a następnie odrzucają żądanie, jeśli użytkownik nie jest zalogowany. Metoda \texttt{GET} filtruje checklisty po właścicielu i stanie archiwizacji, natomiast metoda \texttt{POST} dodatkowo waliduje dane wejściowe, oczyszcza przekazywany obiekt i zapisuje zdarzenie aktywności.
...
Aktualizacja pojedynczej checklisty łączy kilka zabezpieczeń w jednym przepływie. Najpierw sprawdzana jest sesja i poprawność identyfikatora, później walidowana jest treść żądania, a sama aktualizacja wykonywana jest wyłącznie dla dokumentu należącego do aktualnego użytkownika. Warunek \texttt{archivedAt: null} uniemożliwia modyfikację checklist, które zostały już usunięte z aktywnych widoków.
...
Konfiguracja \texttt{authOptions} stanowi centralny punkt mechanizmu autoryzacji. W tym miejscu wskazano adapter MongoDB, strategię sesji, stronę logowania oraz listę aktywnych dostawców OAuth. Callback sesji dopisuje identyfikator użytkownika do obiektu sesji, dzięki czemu pozostałe endpointy mogą korzystać z jednej funkcji pobierającej właściciela danych.
...
\item Dean J., \emph{Web Programming with HTML5, CSS, and JavaScript}, Jones \& Bartlett Learning, Burlington 2017.
