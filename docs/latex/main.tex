...
Diagram przedstawia sekwencję działań wykonywanych podczas przypinania taska do strony głównej aplikacji. Widoczny jest podział odpowiedzialności pomiędzy użytkownika, interfejs aplikacji oraz system zapisujący zmianę. Rysunek pokazuje, że po wybraniu elementu użytkownik otrzymuje informację o aktualnym stanie przypięcia, a następnie może zmienić ten stan za pomocą ikony pinezki.
...
Rysunek pokazuje zmianę wizualną przycisku przypięcia po wykonaniu akcji przez użytkownika. Ikona pinezki zostaje wypełniona kolorem akcentu, dzięki czemu interfejs natychmiast informuje, że checklista jest już przypięta. Takie rozwiązanie ogranicza potrzebę dodatkowych komunikatów tekstowych, ponieważ sam stan przycisku staje się informacją zwrotną.
...
Rysunek przedstawia sekcję przypiętych elementów na dashboardzie. Elementy zostały pokazane jako karty z ikonami typów, tytułami, tagami oraz skróconym podglądem zawartości. Nad kartami znajdują się mechanizmy wyszukiwania, filtrowania i sortowania, dzięki czemu użytkownik może szybko odnaleźć przypiętą checklistę, task, notatkę albo projekt bez przechodzenia do osobnych modułów aplikacji.
...
Rysunek przedstawia pierwszą część formularza tworzenia projektu. Widoczne są pola podstawowe, takie jak tytuł, opis, priorytet, status, termin realizacji oraz tagi. W dalszej części ekranu znajdują się również powiązania z checklistami i notatkami, co pokazuje, że projekt może grupować różne typy informacji potrzebnych do organizacji pracy.
...
Rysunek pokazuje drugą część formularza projektu, w której użytkownik może skonfigurować kolumny tablicy kanban. Każda kolumna jest przedstawiona jako osobny wiersz z nazwą, kolorem, oznaczeniem stanu zakończonego oraz przyciskami zmiany kolejności i usunięcia. Dzięki temu struktura przyszłej tablicy projektu jest widoczna jeszcze przed zapisaniem projektu.
...
Rysunek pokazuje projekt w widoku tablicy kanban. Zadania są rozmieszczone w kolumnach odpowiadających ich statusom, a kolorowe oznaczenia nagłówków pomagają odróżnić poszczególne etapy pracy. Karty tasków zawierają najważniejsze informacje, takie jak tytuł i priorytet, a przyciski przejścia do poprzedniej lub następnej kolumny wspierają zmianę statusu zadania.
